...
def set_loaders(demo=False):
    ...
    if demo:
        train_dataset, test_dataset = torch.utils.data.random_split(test_dataset, [7000, 3000])
    if args.dataset == 'Criteo':
...
def main():
    ...
    train_loader, val_loader = set_loaders(demo=True)
    ...
    for epoch in range(args.epochs):
        model.inferred_correct = 0
        model.inferred_wrong = 0
    ...
    if args.if_cluster_outputsA:
        ...
        outputsA_pca_tsne.fit_transform(np.asarray(outputsA_list))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--half', help='half number of features, generally seen as the adversary\'s feature num. '
                        ...
                        type=int,
                        default=14)  # choices=[16, 14, 32, 1->party_num]. CIFAR10-16, Liver-14, TinyImageNet-32
    ...
    parser.add_argument('-d', '--dataset', default='MNIST', type=str,
                        ...
                        choices=['CIFAR10', 'CIFAR100', 'TinyImageNet', 'CINIC10L', 'Yahoo', 'Criteo', 'BCW', 'MNIST'])
